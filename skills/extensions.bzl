...
def _skills_extension(module_ctx):
    ...
    for mod in reversed(module_ctx.modules):
        for tag in mod.tags.hub:
            if tag.hub_name in hubs:
                fail("skills.bzl currently supports exactly one lockfile per hub; duplicate hub %r" % tag.hub_name)
            hubs[tag.hub_name] = module_ctx.read(module_ctx.path(tag.lockfile))
            if mod.is_root:
                deps = root_module_direct_dev_deps if module_ctx.is_dev_dependency(tag) else root_module_direct_deps
                deps[tag.hub_name] = True

    for hub_name, lockfile_text in hubs.items():
        skills_hub_repository(
            name = hub_name,
            lockfile_json = merge_lockfile_texts([lockfile_text]),
        )
